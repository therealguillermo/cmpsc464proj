...
def test_membership(grammar: Grammar, string: str) -> bool:
    """
    Test if a string belongs to the language generated by a CNF grammar.
    
    Convenience function that creates a MembershipTester instance and tests
    string membership. This is a simple wrapper around the MembershipTester class.
    
    Args:
        grammar (Grammar): The CNF grammar to test against. Must be in CNF.
        string (str): The string to test. Use empty string '' for epsilon.
    
    Returns:
        bool: True if string belongs to the grammar, False otherwise.
    
    Example:
        >>> grammar = parse_grammar_file("grammar.txt")
        >>> test_membership(grammar, "ab")
        True
        >>> test_membership(grammar, "")
        False
    """
    tester = MembershipTester(grammar)
